...
**Daily consumption** is available too — a **daily external statistic** under `eaux_marseille:daily_consumption_<contract>`, plus a precise meter index updated day by day (typically to yesterday). It comes from the web portal's `JOURNEE` chart on contracts that expose it (communicating meters), and — for **SEM** contracts, which the web portal keeps monthly-only — it is sourced automatically from the provider's mobile-app backend instead. Either way it needs no extra setup; contracts with no daily source at all keep the monthly-only behaviour.
