...
La **consommation journalière** est également disponible : une **statistique externe journalière** sous `eaux_marseille:daily_consumption_<contrat>` et un index compteur précis mis à jour au jour le jour (à J-1 en général). Elle provient du graphe `JOURNEE` du portail web sur les contrats qui l'exposent (compteurs communicants), et — pour les contrats **SEM**, que le portail web garde en mensuel — elle est récupérée automatiquement depuis le backend de l'application mobile du fournisseur. Dans les deux cas, aucune configuration supplémentaire n'est nécessaire ; les contrats sans aucune source journalière conservent le comportement mensuel.
